...
const formSchema = z.object({
  ...
  foto_kegiatan: z
    ...
    .optional()
    .refine(
      (files) => files?.length > 0 || typeof files === "undefined",
      "Foto kegiatan harus diunggah"
    ),
...
export default function EditEvent({ currentValue, id, callback }) {
  ...
  const form = useForm({
    ...
    defaultValues: {
      nama_kegiatan: currentValue.nama_kegiatan ?? "",
      tanggal_kegiatan: currentValue.tanggal_kegiatan ?? "",
      foto_kegiatan: undefined, // Foto tidak diisi default karena bersifat opsional
      deskripsi_kegiatan: currentValue.deskripsi_kegiatan ?? "",
    },
  });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <Button disabled={editEvent.isPending} type="submit">
          {editEvent.isPending ? "Menyimpan..." : "Simpan Perubahan"}
        </Button>
      </form>
    </Form>
  );
