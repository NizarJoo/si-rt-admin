...
const TeacherAttendancePage = () => {
  ...
  const fetchTeacherAttendance = useCallback(async () => {
    ...
    try {
      const endpoint =
        viewMode === "daily"
          ? `/auth/dashboard/teacher-attendance/by-date?date=${start}`
          : `/auth/dashboard/teacher-attendance/by-date/recap?start=${start}&end=${end}`;

      const res = await authAxios.get(endpoint);
      console.log("📥 Teacher attendance data:", res.data);
      return res.data;
    } catch (error) {
      throw error; // biar ditangani oleh onError react-query
    }
  }, [authAxios, dateRange, viewMode]);

  const { data, isFetching, refetch } = useQuery({
    queryKey: ["teacher-attendance-list", viewMode, dateRange],
    queryFn: fetchTeacherAttendance,
    enabled:
